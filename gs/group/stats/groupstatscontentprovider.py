...
class GroupStatsContentProvider(SiteContentProvider):
    stats_attributes = (
               ...
               ('average_dialogue_depth', u'Avg. posts in topic'),
               ('min_dialogue_depth', u'Min. posts in topic'),
    ...
    def posts_by_web(self):
        periods = []
        for interval_start, interval_end in self.month_periods:
            result = self.gs.posts_by_web(self.siteInfo.id, self.groupInfo.id,
                                              interval_start,
                                              interval_end)
            periods.append((interval_start.year, interval_start.month, result))
        return periods_as_dict(periods)
